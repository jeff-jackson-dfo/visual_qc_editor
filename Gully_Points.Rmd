...
z3 <- Polygon(cbind(c(dfPoints$long[32:50], dfPoints$long[32]), c(dfPoints$lat[32:50], dfPoints$lat[32])))
z1s <- Polygons(list(z1), "Zone1")
z2s <- Polygons(list(z2), "Zone2")
z3s <- Polygons(list(z3), "Zone3")
SpP = SpatialPolygons(list(z1s, z2s, z3s), 1:3)

# Save the data to a RData file to be used later if desired.
save(dfGully, dfPoints, z1, z2, z3, z1s, z2s, z3s, SpP, file = "gully_points.RData")
